...
def get_tensor(raw_df: pd.DataFrame, y_column_name: str, ocean_column_name: str) -> Tuple[torch.Tensor, torch.Tensor, StandardScaler, StandardScaler]:
    ...
    return X_tensor, y_tensor, x_scaled, y_scaled # 这里返回y_scaled用于后面反归一化得到实际值
...
def predict(
    ...
    model.eval()
    X = x_scaled.transform(raw_input) 
    X_tensor = torch.tensor(X, dtype=torch.float32).to(device)
    
    with torch.inference_mode():
        y_pred = model(X_tensor)
    
    y_real = y_scaled.inverse_transform(y_pred.cpu().numpy())
    return y_real
...
if __name__ == "__main__":
    ...
    X_tensor, y_tensor, x_scaled, y_scaled = get_tensor(raw_df, y_column_name, ocean_column_name)
    ...
    # torch.save(model.state_dict(), "HousingValueModel.pth") 

    model = HousingValueModel(input=X_tensor[0].shape[0], output=1)
    model.load_state_dict(torch.load("HousingValueModel.pth", map_location=device))
    model.to(device)
    ...
    result = predict(model, sample, x_scaled, y_scaled, device)
    print(result)
